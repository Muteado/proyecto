...
class Proyectil():
    ...
    def __init__(self, x, y, ang, velo):
        ...
        self.disparar = False
        self.xreal = x
        self.yreal = ancho[0] - self.y
        self.yUsar = ancho[0] - self.y

    def update(self,x,y):
        "actualizar la posición del proyectil"
        self.velocx = self.veloc * math.cos(math.radians(self.angulo))+Viento_Movimiento[0]
        ...
        if self.disparar is True:
            ...
            self.yreal = (y + self.velocy * self.tiempo +
                          (-(Gravedad[0]) * (self.tiempo ** 2)) / 2)
            # Corregir la posición en el eje vertical
            self.x = self.xreal
            self.y = ancho[0] - self.yreal
        else:
